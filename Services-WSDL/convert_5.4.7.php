...
function callback($input) {
	...
	$returnValue = system("/opt/csw/bin/convert $localname $localname$extension", $returnCode);
	/** \todo The following two parameters should be set in input **/
	//system("/opt/csw/bin/convert -depth 8 $localname $localname$extension");

	if ($returnCode) {
	    error_log('system() failed with error ' . $returnCode);
    ...
    if($extension == '.tif') {
            system("/opt/csw/bin/convert -compress Lossless $localname$extension $localname-2.$extension && mv $localname-2.$extension $localname$extension");
        }
