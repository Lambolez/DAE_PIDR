 ...
 * @author  Tao Sun <tas209@lehign.edu>
 * 
 * @date October 2010
 * @data March 2011 (Modified)
...
$inputT = array();
$inputT['text_file'] = array('name'=>'text_file', 'type'=>'xsd:string');

$outputT = array();
$outputT['ner_tagged_file'] = array('name'=>'ner_tagged_file', 'type'=>'xsd:string');
//== STOP EDITABLE ZONE
...
function callback($input) {
	...
	//The absolue path of stanford-ner.jar and classifiers should be changed according your environment configuration
	$execString = "/dae/database/upload/manual/stanford-ner/ner.sh $localname $localOutput";
	//== STOP EDITABLE ZONE
	
	$returnString = system($execString, $returnCode);
	if ($returnCode) {
	    return new soap_fault('SERVER', '', 'Execution Error', 'Return code = ' . $returnString);
	}
