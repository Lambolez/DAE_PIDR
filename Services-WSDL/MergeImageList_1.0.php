 ...
 */
$inputT = array();
$inputT['ImageList'] = array('name'=>'ImageList', 'type'=>'tns:StringArray');
...
$outputT['mergedImage'] = array('name'=>'mergedImage', 'type'=>'xsd:string');
...
function callback($input) {
	...
	foreach(array_keys($input) as $inputFieldName) {
		...
		if(is_array($input[$inputFieldName])) {
	        ...
	        foreach(array_keys($input[$inputFieldName]) as $inputItem) {
		    	$inputValues[$inputFieldName][$inputItem] = getRemoteArguments($input[$inputFieldName][$inputItem],$localdir);
	        }
	    }
	    else if (! is_null($input[$inputFieldName])) {
	      $inputValues[$inputFieldName] = getRemoteArguments($input[$inputFieldName],$localdir);
	    }	
	 ...
	 * 
	 */
	$localOutput['mergedImage'] = realpath($localdir) .  '/output.pbm';
    $logFile = escapeshellarg(realpath($localdir) .  '_logfile.txt');
	
	$execString = "x=\"\" ; for i in `find " .escapeshellarg(realpath($localdir)) ." -type f` ; do if [ -z \"\$x\" ] ; then x=\"\$i\"; continue; else $algopath \$x \$i -compose Darken -composite ". $localOutput['mergedImage'] ." ; x=" .$localOutput['mergedImage'] ." ; fi ; done > $logFile 2>&1";
	//== STOP EDITABLE ZONE
...
function getRemoteArguments($argument,$localdir) {
