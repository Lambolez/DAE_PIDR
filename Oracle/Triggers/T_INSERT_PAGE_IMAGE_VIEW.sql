create or replace
TRIGGER "DEZHAO"."T_INSERT_PAGE_IMAGE_VIEW" 
INSTEAD OF INSERT ON PAGE_IMAGE_VIEW
declare
...
BEGIN
  imageID := :new.id;
  --manage the metadata of a page image
  insert into data_item (id, description, flag) values (imageID, :new.description, 'page_image');
  insert into image_data_item (id) values (imageID);
  insert into physical_image_data_item (id) values (imageID);
  insert into page_image (id, vdpi, hdpi, width, height, skew, path) 
    values (imageID, :new.vdpi, :new.hdpi, :new.width, :new.height, :new.skew, :new.path);
  
  --manage a page image's contributor list
  ...
  if(contributor_list is not null and contributor_list.last>0) then
    for i in contributor_list.first .. contributor_list.last loop
      ...
      insert into contribute (person_id, data_item_id, description, type) 
        values (contributor.person_id, imageID, contributor.description, contributor.type);
